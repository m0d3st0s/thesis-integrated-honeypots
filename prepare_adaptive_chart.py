...
try:
    ...
    has_cowrie = ".Values.honeypots.cowrie.image.repository" in original
    has_dionaea = ".Values.honeypots.dionaea.image.repository" in original
    if not (has_cowrie or has_dionaea):
        raise ValueError("No supported honeypot container found.")

    if not has_dionaea:
        if "dionaea-state" in original:
            raise ValueError("Unexpected Dionaea storage in a non-Dionaea chart.")
        changes = changes[:1]
    ...
    if all(new in original for old, new in changes):
        ...
        print("Chart prepared; Recreate enabled; Dionaea persistence:", has_dionaea)
