...
try:
    ...
    if plan.get("status") != "compatible_with_current_mixed_layout":
        raise ValueError("Plan is not compatible with the current mixed layout.")
    ...
    if set(mappings) != set(EXPECTED):
        raise ValueError("This deployment requires both SSH and HTTP.")
